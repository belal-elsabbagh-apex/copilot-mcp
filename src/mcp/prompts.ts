...
export function registerPrompts(server: McpServer): void {
    ...
    ({ profile }) => {
      return userText(
        `Reconcile account settings between prod and pre-prod for profile="${profile}".\n` +
          `1. Call list_setting_sections to see which sections/tags can be compared.\n` +
          `2. Call diff_settings (profile="${profile}") for the relevant tags to find drift, focusing on each section's onlyInProd items (present in prod, missing in pre-prod).\n` +
          `3. Call plan_settings_sync (profile="${profile}", same tags/sections scope) to compute the additive actions. It is READ-ONLY and returns each action with a stable id, a one-line summary, and warnings about references dropped for having no pre-prod match by name.\n` +
  ...
  server.registerPrompt(
    "inspect-settings",
    {
      ...
      description:
        "Fetch and summarize an account's settings from a single env (prod or pre_prod), optionally scoped to a tag.",
      argsSchema: {
        profile: completable(
          z.string().min(1).describe("Credential profile / account name from config (required)"),
          () => listProfiles(),
        ...
        tag: z.string().optional().describe("Settings tag to focus on (e.g. 'orders')"),
      },
    },
    ({ profile, env, tag }) => {
      const envArg = env ? `, env="${env}"` : "";
      const tagArg = tag ? `, tags=["${tag}"]` : "";
      const envReminder = env
        ? ""
        : " get_settings requires an env (prod or pre_prod) — confirm with the user which env to inspect before calling it.";
      return userText(
        `Inspect Copilot settings for profile="${profile}"${env ? ` in ${env}` : ""}${tag ? ` (tag: ${tag})` : ""}. Everything here is READ-ONLY.\n` +
          `1. Call list_setting_sections${tag ? ` with tag="${tag}"` : ""} to see the available sections; note which are 'derived' (crawled from order types — heavier to fetch).\n` +
          `2. Call get_settings (profile="${profile}"${envArg}${tagArg}) to fetch the sections from that one env.\n` +
